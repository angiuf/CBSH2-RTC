#!/usr/bin/env python3
"""
Test script for CBSH2-RTC algorithm on warehouse environments.
Reads environment cases, creates temporary .scen files, runs the cpp CBSH2-RTC code,
...
def parse_paths_output(paths_file_path, num_agents):
    """Parse the paths output file from CBSH2-RTC."""
...
def run_cbsh2_rtc(map_file, scenario_file, output_csv, paths_file, num_agents, time_limit=60):
    """Run the CBSH2-RTC executable and return results."""
...
def parse_output_csv(csv_file_path):
    """Parse the output CSV file from CBSH2-RTC and extract only runtime."""
...
def run_single_test(dataset_path, map_name, num_agents, test_id, time_limit=60):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Run CBSH2-RTC
        success, actual_time, stdout, stderr = run_cbsh2_rtc(
...
def test_cbsh2_rtc():
    """Main testing function for CBSH2-RTC."""
    # Base paths
    base_dir = Path(__file__).parent
    project_dir = base_dir.parent.parent
    dataset_path = project_dir / 'baselines/Dataset'
    model_name = "CBSH2-RTC"
